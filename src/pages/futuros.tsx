...
const Futuros: NextPage<FuturosProps> = ({
  ...
  // Filtrar e ordenar todas as oportunidades válidas
  let validOperations = allArbitrageOpportunities
    .filter((operation: ImportedArbitrageOpportunity) => {
      return (
        operation &&
        operation.spread > 0 &&
        operation.highestBid?.exchange &&
    ...
    .sort(
      (a: ImportedArbitrageOpportunity, b: ImportedArbitrageOpportunity) =>
        b.spread - a.spread
    );

  // Manter um registro de pares de exchanges já processados para cada moeda
  const processedPairs = new Set<string>();
  let sortedOperations = validOperations.filter(
    (operation: ImportedArbitrageOpportunity) => {
      const pairKey = `${operation.ticker}_${operation.highestBid.exchange}_${operation.lowestAsk.exchange}`;

      // Se esse par específico de exchanges para essa moeda já foi processado, ignorar
      if (processedPairs.has(pairKey)) {
        return false;
      }

      // Registrar esse par de exchanges para essa moeda
  ...
  if (orphanCoins.length > 0 && (!isAdmin || !isNewUser)) {
    sortedOperations = sortedOperations.filter(
      (operation: ImportedArbitrageOpportunity) => {
  ...
  const updatedOperations = useMemo(() => {
    return (sortedOperations ?? [])
      .map((operation: ImportedArbitrageOpportunity) => {
        ...
        if (
          ...
          const newSpread =
            ((validBidPrice - validAskPrice) / validAskPrice) * 100;
          return {
            ...operation,
            spread: newSpread,
            lowestAsk: {
